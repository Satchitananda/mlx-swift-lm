...
func stackedQuantizedLinear(_ layers: [QuantizedLinear]) -> QuantizedLinear? {
    guard let first = layers.first,
        layers.allSatisfy({
            // This fork also supports a global NVFP4 scale. Rebuilding a plain
            // stacked layer would drop it; leave these projections untouched.
            $0.globalScale == nil && $0.bias == nil && $0.groupSize == first.groupSize && $0.bits == first.bits
                && $0.mode == first.mode
        })
    else { return nil }
